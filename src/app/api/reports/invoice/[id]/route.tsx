...
export const GET = async (
	...
	const idFromRepresent = Number.isNaN(+id);
	...
	if (!idFromRepresent) {
		const [invoicesResult] = await db
			.select()
			.from(invoices)
			.where(and(eq(invoices.id, +id), eq(invoices.verified, true)));
		...
		const [athlete] = await db
			...
			.where(eq(athletes.id, invoicesResult.athlete_id));
		...
		const [represent] = await db
			...
			.from(representatives)
			.innerJoin(users, eq(representatives.user_id, users.id))
			.where(eq(representatives.id, invoicesResult.representative_id));
		...
		const stream = await renderToStream(
			<InvoicePDF
				invoiceData={{
					...
					representative_id: represent as Representative,
					price: +pricing,
				}}
			/>,
		);
